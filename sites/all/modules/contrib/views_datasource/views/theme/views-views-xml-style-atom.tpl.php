...
foreach ($entries as $entry) {
  if (!((array_key_exists("id", $entry)) && array_key_exists("title", $entry) && array_key_exists("updated", $entry))) {
    ...
    elseif ($options['using_views_api_mode']) {
      ...
      drupal_set_message(t('Either the id, title, or updated attribute is missing.'), 'error');
    }
    return;
  }
  $id = $entry["id"];
  ...
  if (array_key_exists("author", $entry)) {
    ...
    if (array_key_exists("email", $entry["author"])) {
  ...
  foreach ($entry as $l => $v) { //Then the rest
    if (($l != "id") && ($l != "title") && ($l != "updated") && ($l != "author") && ($l != "link") && ($l != "teaser") && ($l != "content")) {
      if (is_array($v)) {
        ...
        $xml .= "    <$l>" . (($options['escape_as_CDATA'] == 'yes') ? "<![CDATA[$v]]>" : "$v") . "</$l>\n";
      }
    }
  }
  $xml .= "  </entry>\n";
...
if ($view->override_path) {       // inside live preview
  ...
  if ($options['using_views_api_mode']) {     // We're in Views API mode.
    print $xml;
  }
  else {
